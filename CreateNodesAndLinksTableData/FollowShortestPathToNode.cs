...
namespace SimioTravelSteeringBehaviors
{
    public class FollowShortestPathToNodeDefinition : ITravelSteeringBehaviorDefinition
    {
        ...
        public string Name
        {
            get { return "Follow Network Path Same Network Object Avoidance"; }
        }
        ...
        {
            get { return "Steers an entity to follow a specified network path from a starting node to an ending node, staying within the boundaries of a specified path width. Only avoid objects that are in the same network."; }
        }
        ...
        static readonly Guid MY_ID = new Guid("{c4ad4771-06bf-4369-bca3-2755f9a045c7}");
    ...
    class FollowShortestPathToNodeManager
    {
        ...
        const string DATA_KEY = "3172ed86-699d-4a92-a9bb-cac308166a6d";
        ...
        {
            var i = info.Original as IEntityRuntimeData;
            ...
            for (double x = topLeft.X; x <= bottomRight.X; x++)
            {
                for (double z = topLeft.Z; z <= bottomRight.Z; z++)
                {
                    ...
                    if (_gridLocationToEvasionTargetInfoMap.TryGetValue(new FacilityLocation(x, 0, z), out evasionTargetInfos))
                    {
                        ...
                        {
                            var e = evTarget.Original as IEntityRuntimeData;

                            if (e != null && i != null)
                            {
                                // Return everything but the entity making the request
                                if (evTarget.Original != info.Original && e.CurrentNetwork == i.CurrentNetwork)
